...
SELECT 
    ...
    (SELECT AVG(isp_nvl_ctn) 
     FROM ensj_staging_m_tb_einsp_njma061_crcr_isp_ifo_h 
     WHERE isp_atc_id = 'SPCWND1077') AS "SEPA-ANODE GAP MC SIDE",

    (SELECT AVG(isp_nvl_ctn) 
     FROM ensj_staging_m_tb_einsp_njma061_crcr_isp_ifo_h 
     WHERE isp_atc_id = 'SPCWND1092') AS "SEPA-ANODE GAP OP SIDE",
